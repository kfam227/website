# apply_media.ps1 — PowerShell port of apply_media.py
# Usage: .\apply_media.ps1 <input.html> <output.html>

param(
    [string]$InputPath = "NMThera_BusinessPlan_2026_v6.html",
    [string]$OutputPath = "business-plan-enhanced.html"
)
...
Write-Host "Read $($html.Length.ToString('N0')) characters from $InputPath"
...
$html = $html.Replace('</style>', $new_css + "`n</style>")
...
# ═══════════════════════════════════════════════════════════════
$cover_video = @'
<div class="cover-video-wrap" aria-hidden="true">
...
$html = $html.Replace(
    ...
    $cover_video + "`n  " + '<div class="cover-caustic" aria-hidden="true">'
)
...
$html = $html.Replace(
    ...
    $s1_image + "`n    " + '<p>The answer took eight years to build.'
)
...
$html = $html.Replace(
    ...
    $s2_image + "`n    " + '<p>The options for treating conditions like palmar hyperhidrosis have barely evolved'
)

# ═══════════════════════════════════════════════════════════════
# 6a. S3 PRODUCT — Pool water video
# ═══════════════════════════════════════════════════════════════
$s3_video = @'
...
$html = $html.Replace(
    ...
    $s3_video + "`n    " + '<h3>The Engineering Core'
)

# ═══════════════════════════════════════════════════════════════
# 6b. S3 PRODUCT — Patent drawing pair
# ═══════════════════════════════════════════════════════════════
$s3_patent = @'
...
$html = $html.Replace(
    ...
    $s3_patent + "`n    " + '<h3>The Universal Port Interface</h3>'
)

# ═══════════════════════════════════════════════════════════════
# 6c. S3 PRODUCT — Fist image near platform expansion
# ═══════════════════════════════════════════════════════════════
$s3_fist = @'
...
$html = $html.Replace(
    ...
    $s3_fist + "`n    " + '<h3>Platform Expansion Arc</h3>'
)
...
$html = $html.Replace(
    ...
    $s4_image + "`n    " + '<h3>The Volume Advantage'
)

# ═══════════════════════════════════════════════════════════════
# 8a. S5 LAUNCH — Travel Kit video
# ═══════════════════════════════════════════════════════════════
$s5_video = @'
...
$html = $html.Replace(
    ...
    $s5_video + "`n    " + '<h3>Early Proof Points</h3>'
)

# ═══════════════════════════════════════════════════════════════
# 8b. S5 LAUNCH — Product image pair
# ═══════════════════════════════════════════════════════════════
$s5_products = @'
...
$html = $html.Replace(
    ...
    $s5_products + "`n    " + '<div class="batch-alloc">'
)
...
$s6_old = '<div class="roadmap-track">
      <div class="track-head">
        <div class="track-dot dot-r"></div>'

$s6_new = $s6_images + "`n    " + '<div class="roadmap-track">' + "`n      " + '<div class="track-head">' + "`n        " + '<div class="track-dot dot-r"></div>'
...
$html = $html.Replace(
    ...
    $s7_patent + "`n    " + "<p>NM Thera's IP portfolio is a material competitive moat."
)
...
$html = $html.Replace(
    ...
    $s11_image + "`n    " + '<h3>Clinical Dispensing Opportunity</h3>'
)
...
$html = $html.Replace(
    ...
    $s12_image + "`n    " + '<p>SOKY enters the market as the only wearable soak platform'
)

# ═══════════════════════════════════════════════════════════════
# 13. S9 PARTNERS — Fingers splayed image
# ═══════════════════════════════════════════════════════════════
$s9_image = @'
...
$html = $html.Replace(
    ...
    $s9_image + "`n    " + '<div class="callout green"><div class="callout-lbl">Rapid Liquid Printing (RLP)'
)
...
$html = $html.Replace(
    ...
    $s14_image + "`n    " + '<div class="analyst-quote">'
)

# ═══════════════════════════════════════════════════════════════
# 15. S8 TEAM — Hands clasped + side fist images
# ═══════════════════════════════════════════════════════════════
$s8_image = @'
...
$s8_old = '<h3>Advisors</h3>
    <div class="people-grid">
      <div class="person hl-pool">
        <span class="person-badge badge-advisor">Research Advisor'

$s8_new = $s8_image + "`n    " + '<h3>Advisors</h3>' + "`n    " + '<div class="people-grid">' + "`n      " + '<div class="person hl-pool">' + "`n        " + '<span class="person-badge badge-advisor">Research Advisor'
...
$html = $html.Replace(
    ...
    $s10_image + "`n    " + '<h3>Regulatory Pathway</h3>'
)

# ═══════════════════════════════════════════════════════════════
# 17. COVER — NM Thera logo in cover-top
# ═══════════════════════════════════════════════════════════════
$html = $html.Replace(
...
$html = $html.Replace('</script>', $mute_js + "`n</script>")
...
$inputLen = (Get-Item $InputPath).Length
$outputLen = (Get-Item $OutputPath).Length
$added = $outputLen - $inputLen

Write-Host "Wrote $($outputLen.ToString('N0')) characters to $OutputPath"
Write-Host "Added approximately $($added.ToString('N0')) bytes of media content"
Write-Host "Media integrations applied: 18 assets across 14 sections"
Write-Host "Done!"
